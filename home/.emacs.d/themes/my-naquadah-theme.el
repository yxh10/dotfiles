;;; my-naquadah-theme.el --- A theme based on Tango color set
...
(defun my-naquadah-get-colors (name)
  (cdr
   (assoc
    name
    (car my-naquadah-colors))))

(defun my-naquadah-simple-face-to-multiple (face)
  (let ((spec (car face))
        (lst (cadr face)))
    (list spec (mapcar
                (lambda (entry)
                  (let ((color-condition (car entry)))
                    (list color-condition
                          (my-naquadah-color-list-expand (cdr entry) lst))))
                my-naquadah-colors))))

(defun my-naquadah-color-list-expand (color-alist lst)
  (let ((result '()))
    (while (car lst)
      (let ((key (car lst))
            (val (cadr lst)))
        ...
        (if (listp val)
            (setq val (my-naquadah-color-list-expand entry val)))
        (setq result (append result `(,key ,val))))
      (setq lst (cddr lst)))
    result))

(defun my-naquadah-properties-get (lst prop)
  (cadr (assoc prop lst)))

(defun my-naquadah-apply-unspecified-properties (inherit-props props)
...
(defun my-naquadah-append-unspecified-properties (lst)
  (dolist (elt lst)
    (let ((key (car elt))
          (props (cadr elt)))
        ...
        (let ((inherit-prop (plist-get props :inherit)))
          (if (listp inherit-prop)
              (dolist (i inherit-prop)
                (my-naquadah-apply-unspecified-properties (my-naquadah-properties-get lst i) props))
            (my-naquadah-apply-unspecified-properties (my-naquadah-properties-get lst inherit-prop) props))))))
  lst)

(defun my-naquadah-theme-set-faces (theme &rest args)
  (apply 'custom-theme-set-faces
         (append (list theme)
                 (mapcar 'my-naquadah-simple-face-to-multiple
                         (my-naquadah-append-unspecified-properties args)))))

(my-naquadah-theme-set-faces
 'my-naquadah
 '(ido-first-match (:foreground "orange" :bold t))
 '(ido-only-match (:foreground "orange" :bold t))
 ...
 '(cursor (:background aluminium-3))
 '(hl-line (:background sky-blue-3))
...
(provide-theme 'my-naquadah)

;; Local Variables:
;; no-byte-compile: t
;; End:

;;; my-naquadah-theme.el ends here
